	...
	)
{
  ...
  save_result = SOIL_save_image_to_stream(stream, image_type, width, height, channels, data);
	fclose(stream);
	...
	)
{
  ...
  switch(image_type)
  {
  case SOIL_SAVE_TYPE_BMP:
		save_result = stbi_write_bmp( stream, width, height, channels, (void*)data );
    break;
  case SOIL_SAVE_TYPE_TGA:
		save_result = stbi_write_tga( stream, width, height, channels, (void*)data );
    break;
  case SOIL_SAVE_TYPE_DDS:
		save_result = save_image_as_DDS( stream, width, height, channels, data );
    break;
  default: save_result = 0;
  }
  	
	result_string_pointer =  save_result == 0 ? "Saving the image failed":"Image saved";
	return save_result;
}
